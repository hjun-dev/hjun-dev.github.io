...
로켓 착륙 제어는 비선형적이고 underactuated한 로켓을 제한된 연료 안에서 실시간으로 제어해야 하는 문제이다. 또한 착륙 순간에는 위치, 속도, 자세, 각속도 등의 terminal constraint를 만족해야 한다. 따라서 일반적인 강화학습 알고리즘을 그대로 적용하기 어렵다. <br>
...
\operatorname{clip}(\rho(\theta), 1-\epsilon, 1+\epsilon) A^{\pi_{old}}(x,u)
\right)
$$
...
a'
=
\operatorname{clip}(a + k\Delta a, -1, 1)
$$
